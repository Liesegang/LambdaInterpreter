...
class FreeVariables(ast.NodeVisitor):
    ...
    determines which variable (if any) are bobuded. Ultimately provides a
    set of string variable names.
    """
...
class BoundVariables(ast.NodeVisitor):
    ...
    This visitor traverses a lambda culculus abstrac syntax tree and provides
...
class AlphaConversion(ast.NodeVisitor):
    ...
    Attributes:
        to_replace (Variable): Instance whose name attribbute must match the
    ...
    def visit_Variable(self, node):
        """If the appropriate variable name is found, replaced it."""
    ...
    def visit_Abstraction(self, node):
        ...
        if node.parameter.name == self.to_replace.name:
        ...
        if node.parameter.name in FreeVariables().visit(self.replacement):
            ...
            new_parameter = Variable(next(s for s in lexicographical() if s not in unavailable_names))
...
class BetaReduction(ast.NodeVisitor):
    ...
    sematics, (λx.M) N -> M[N/x]. This visitor provides a new abstract syntax
    ...
    def visit_Variable(self, node):
        """Clone the given Variable node."""
        return Variable(node.name)

    def visit_Application(self, node):
        """Performs the application if the left-hand side reoresents an
